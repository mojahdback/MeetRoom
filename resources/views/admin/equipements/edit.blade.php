...
<body class="bg-gray-100 min-h-screen p-8">
    ...
    <div class="max-w-lg mx-auto">
        ...
        <div class="bg-white rounded-2xl shadow-sm p-8">
            ...
            <form method="POST" action="{{ route('admin.equipements.update', $equipement) }}" class="space-y-4">
                @csrf
                @method('PATCH')
                ...
                <div>
                    ...
                    <select name="type"
                        ...
                        <option value="audio" {{ old('type', $equipement->type) == 'audio' ? 'selected' : '' }}>Audio
                        </option>
                        <option value="video" {{ old('type', $equipement->type) == 'video' ? 'selected' : '' }}>Vidéo
                        </option>
                        <option value="display" {{ old('type', $equipement->type) == 'display' ? 'selected' : '' }}>
                            Display</option>
                        <option value="autre" {{ old('type', $equipement->type) == 'autre' ? 'selected' : '' }}>Autre
                        </option>
                    </select>
